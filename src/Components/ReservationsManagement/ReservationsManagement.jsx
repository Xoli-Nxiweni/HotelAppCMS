...
const ReservationsManagement = () => {
  ...
  useEffect(() => {
    const fetchReservations = async () => {
      try {
        ...
        showSnackbar('Error fetching reservations', 'error', error);
      }
    };
    fetchReservations();
  }, []);
  ...
  const handleAdd = async () => {
    if (newReservation) {
      try {
        ...
        showSnackbar('Error adding reservation', 'error', error);
      }
    }
  };
  ...
  const handleUpdate = async () => {
    if (editReservation.id && editReservation.name) {
      try {
        ...
        showSnackbar('Error updating reservation', 'error', error);
      }
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      showSnackbar('Error deleting reservation', 'error', error);
    }
  };
